...
class GamificationControllerTest {
        ...
        @Test
        void getAdminAchievements_shouldReturn403_whenCurrentUserIsNotAdmin() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.USER);

                mockMvc.perform(get("/gamification/admin/achievements")
                                .header("Authorization", "Bearer valid-token"))
                                .andExpect(status().isForbidden());

                verify(gamificationService, never()).getAchievementDefinitionsForAdmin();
        }

        @Test
        void getAdminAchievements_shouldReturnData_whenCurrentUserIsAdmin() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.ADMIN);
                when(gamificationService.getAchievementDefinitionsForAdmin()).thenReturn(List.of(
                                AchievementDefinitionDto.builder()
                                                .code("CUMULATIVE_EXAM_ATTEMPTS_3")
                                                .name("Khởi động thi cử")
                ...
                mockMvc.perform(get("/gamification/admin/achievements")
                                .header("Authorization", "Bearer valid-token"))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$[0].code").value("CUMULATIVE_EXAM_ATTEMPTS_3"))
                                .andExpect(jsonPath("$[0].ruleType").value("CUMULATIVE_EXAM_ATTEMPTS"));

                verify(gamificationService).getAchievementDefinitionsForAdmin();
        }

        @Test
        void createAdminAchievement_shouldReturn400_whenPayloadInvalid() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.ADMIN);

                mockMvc.perform(post("/gamification/admin/achievements")
                                .header("Authorization", "Bearer valid-token")
                ...
                verify(gamificationService, never()).upsertAchievementDefinition(any());
        }

        @Test
        void createAdminAchievement_shouldDelegateToService_whenPayloadValidAndAdmin() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.ADMIN);
                when(gamificationService.upsertAchievementDefinition(any())).thenReturn(
                                AchievementDefinitionDto.builder()
                ...
                mockMvc.perform(post("/gamification/admin/achievements")
                                ...
                                .content("""
                                                ...
                                                """))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.code").value("CUMULATIVE_EXAM_ATTEMPTS_20"));

                verify(gamificationService).upsertAchievementDefinition(any());
        }

        @Test
        void updateAdminAchievement_shouldUsePathCode_whenCurrentUserIsAdmin() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.ADMIN);
                when(gamificationService.upsertAchievementDefinition(any())).thenReturn(
                                AchievementDefinitionDto.builder()
                                                .code("STREAK_DAYS_5")
                ...
                mockMvc.perform(put("/gamification/admin/achievements/STREAK_DAYS_5")
                                .header("Authorization", "Bearer valid-token")
                                .contentType(APPLICATION_JSON)
                                .content("""
                                                {
                                                ...
                                                """))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.code").value("STREAK_DAYS_5"));

                verify(gamificationService)
                                .upsertAchievementDefinition(argThat(request -> "STREAK_DAYS_5".equals(request.code())
                                                && "STREAK_DAYS".equals(request.ruleType())
                                                && Integer.valueOf(5).equals(request.ruleThreshold())));
        }

        @Test
        void deleteAdminAchievement_shouldReturn204_whenCurrentUserIsAdmin() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.ADMIN);

                mockMvc.perform(delete("/gamification/admin/achievements/STREAK_DAYS_5")
                                .header("Authorization", "Bearer valid-token"))
                                .andExpect(status().isNoContent());

                verify(gamificationService).deleteAchievementDefinition("STREAK_DAYS_5");
        }

        @Test
        void assignAdminAchievement_shouldReturn400_whenUserIdMissing() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.ADMIN);

                mockMvc.perform(post("/gamification/admin/achievements/STREAK_DAYS_5/assign")
                                .header("Authorization", "Bearer valid-token")
                                .contentType(APPLICATION_JSON)
                                .content("{}"))
                                .andExpect(status().isBadRequest());

                verify(gamificationService, never()).assignAchievementToUser(any(), any());
        }

        @Test
        void assignAdminAchievement_shouldReturn403_whenCurrentUserIsNotAdmin() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.USER);
        ...
        @Test
        void assignAdminAchievement_shouldDelegateToService_whenCurrentUserIsAdmin() throws Exception {
                when(userService.getCurrentUserRole()).thenReturn(Role.ADMIN);
                ...
                verify(gamificationService).assignAchievementToUser("STREAK_DAYS_5", 99L);
        }
